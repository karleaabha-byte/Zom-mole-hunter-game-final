...
class MoleAI:
    ...
    def decide_help_or_sabotage(
        ...
        Zephyr scores both candidate actions with the utility
        function, applies a fixed penalty representing the
        detective's anticipated response, and picks whichever
        action has the higher resulting utility.
        """
    ...
    def _detective_response(
        ...
        """
        Model the detective's anticipated counter-response.

        The detective prefers outcomes that make Zephyr's
        position worse, so we take whichever modeled response
        yields the lower utility for Zephyr.
        ...
        try:
            ...
            # Detective favors whichever modeled response
            # produces the lower utility for Zephyr.
            return min(
                investigate_score,
                scrutinize_score
            )
